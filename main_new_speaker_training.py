    ...
    for (optim_name, lr, scheduler_name, weight_decay, use_bn, dropout_p, skip) in yield_hyperparameters(hyperparameters):
        ...
        class Config:
            trainer_params = {
                ...
                'seed': 83,
                'n_epochs': 1000,
                'exp_name': f'new_speaker_voices_detection_{optim_name=}_{lr=}_{weight_decay=}_{scheduler_name=}_{activation=}_{dropout_p=}_{use_bn=}_{is_bn_pre_act=}_{skip=}', # nazwa eksperymentu, która będzie użyta do tworzenia folderów i logowania
            ...
            data_params = {
                ...
                'dataset_params': {
                    ...
                    'df_train_path': 'data/1new_speaker_train_spectogram_df.csv',
                    'df_val_path': 'data/1new_speaker_val_spectogram_df.csv',
                    'df_test_path': 'data/1new_speaker_test_spectogram_df.csv',
            ...
            model_params = {
                ...
                'checkpoint_path': '/net/people/plgrid/plgkrzepk/newGithub/FCV_project/models/best_model_adamw_0.001_0.1_ReLU_0.0_True_True_False.pt', # path to load model checkpoint
                'init': None,
                'freeze_backbone': True,  # whether to freeze the backbone of the model
            }
            ...
            extra_modules_params = {
                'early_stopping': {
                    ...
                    'mode': 'max',
                    'delta': 1e-4,
                    'checkpoint_path': f"models/best_model__one_speaker_{optim_name}_{lr}_{weight_decay}_{activation}_{dropout_p}_{use_bn}_{is_bn_pre_act}_{skip}.pt",
